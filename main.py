...
import src.utils.retrieve
import src.utils.generate
...
def main():
	if args.option in ["add","a"]:
		...
		if res is not None:
			src.utils.add.addEntry(res[0],res[1],args.name,args.url,args.email,args.login)
	...
	if args.option in ["extract","e"]:
		...
		if res is not None:
			src.utils.retrieve.retrieveEntries(res[0],res[1],search,decryptPassword = args.copy)
	...
	if args.option in ["generate","g"]:
		if args.length == None:
			...
			return
		password = src.utils.generate.generatePassword(args.length)
